...
async function checkMatchingPassword(account_email, account_password){
  try {
    ...
    let hashedPassword = await pool.query(sql, [account_email])
    return bcrypt.compareSync(account_password, hashedPassword.rows[0].account_password)
  } catch (error) {
    return error.message
  }
}
